    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--surface)' }}>
      <div className="flex-1 flex flex-col justify-center px-6 py-12">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-2">Crear cuenta</h2>
          <p className="text-[var(--text-secondary)] text-sm">Empieza a controlar tus finanzas hoy.</p>
        </div>
        ...
        <form action={handleRegister} className="flex flex-col gap-4">
          {[
            ...
            <div key={f.id} className="flex flex-col gap-1.5">
              <label htmlFor={f.id} className="text-[11px] font-semibold uppercase tracking-widest text-[var(--text-secondary)]">
                {f.label}
              </label>
          ...
          {['password', 'confirm_password'].map((name, i) => (
            <div key={name} className="flex flex-col gap-1.5">
              <label htmlFor={name} className="text-[11px] font-semibold uppercase tracking-widest text-[var(--text-secondary)]">
