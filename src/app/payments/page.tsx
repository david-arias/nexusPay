  ...
  return (
    <div className="flex flex-col min-h-screen pb-24" style={{ backgroundColor: 'var(--surface)' }}>
      <header className="flex items-center justify-between px-4 pt-12 pb-2">
        ...
        <button className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-[var(--card-hover)] tap-none">
      ...
      <div className="mx-4 mt-3 bg-blue-600 rounded-2xl p-5">
        ...
        <div className="mt-3 h-1.5 bg-[var(--card)]/20 rounded-full overflow-hidden">
          <div className="h-full bg-[var(--card)] rounded-full" style={{ width: totalAll > 0 ? `${(totalPaid / totalAll) * 100}%` : '0%' }} />
        </div>
      </div>
                ...
                return (
                  <Link key={entry.id} href={`/payments/${entry.id}`}
                    ...
                    <div className="text-right">
                      <p className={cn('font-bold tabular-nums', isPaid ? 'text-[var(--text-secondary)]' : 'text-[var(--text-primary)]')}>
